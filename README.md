...
This repository includes an explicit **numerical assumption–verification pipeline**
corresponding exactly to the hypotheses used in **Appendix A** of the paper.
No new theoretical results are introduced here; the purpose of these files is
to document, in a fully reproducible manner, that the numerical experiments
operate within the regime assumed by the geometric–information correspondence.

### Assumption tracker script

* **`gi_assumption_tracker_v3.py`**

  This script implements the numerical pipeline underlying Appendix~A.
  For increasing histogram resolutions $N=b^2$ and fixed mollification bandwidth
  $\sigma>0$, it performs:
  - reconstruction of geometric point clouds at each resolution,
  - entropic optimal-transport matching followed by Procrustes alignment,
  - construction of mollified empirical histograms,
  - execution of the GI--flow in fixed-$T$ or adaptive stopping mode.

  The script records numerical diagnostics verifying that, uniformly across
  resolutions,

  (i) the GI--flow residual
  \[
    \delta_n := D_{\mathrm{KL}}\bigl(P_M^{(n)} \| X^{C\to M}_{T_n}\bigr)
  \]
  is small and decreases under iteration;

  (ii) the total variation distance
  $\|P_C^{(n)}-P_M^{(n)}\|_{\mathrm{TV}}$ remains controlled after mollification;

  (iii) the histogram overlap
  $\sum_i \min(P_C^{(n)}(i),P_M^{(n)}(i))$ remains high; and

  (iv) no probability mass escapes the fixed sampling window.

  This script supersedes earlier experimental variants and is the one referenced
  explicitly in Appendix~A of the paper.

### Representative numerical outputs

The following CSV/JSON files are included as *reference artifacts* documenting
the numerical regime assumed in Appendix~A:
...
### Reproducing Table~A.1 (Appendix A)

To regenerate the numerical diagnostics table reported in Appendix~A, run:

```bash
python gi_assumption_tracker_v3.py \
  --module scripts/tci_construct_mandelbrot_v002_fixed.py \
  --sigma-bins 3.0 \
  --T-fixed 25 \
  --bins-start 64 --bins-max 512 \
  --out-prefix v3_T25_sigma3_dense
```
